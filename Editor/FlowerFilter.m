...
(
 ...
 vec4 blendColor(sampler2D sucai, vec4 baseColor,vec2 videoSize,vec2 sucaiSize,vec2 anchorImageCoord,float sucaiScale)
 {
     ...
     vec2 sucaiUV = sucaiAlign(textureCoordinate2,videoSize,sucaiSize,anchorImageCoord,sucaiScale);
...
//             return fgColor;
 ...
 void main(void)
 {
     vec2 baseTexureSize = vec2(200.0,201.0);
     ...
     lowp vec4 fullblendColor = blendColor(inputImageTexture2,baseColor,baseTexureSize,fullBlendTexSize,
         fullBlendAnchor,1.0);
...
- (id)init;
{
    ...
    sizeUniform = [filterProgram uniformIndex:@"fullBlendTexSize"];
    
    glUniform2f(sizeUniform, 300.0, 300.0);
