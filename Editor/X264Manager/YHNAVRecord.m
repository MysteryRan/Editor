...
typedef enum FFMAVMediaType FFMAVMediaType;


@implementation YHNAVRecord
{
...
- (AVStream*) AddStream: (FFMAVMediaType) codec_type {
...
- (void)encodesss:(CVPixelBufferRef)pixelBuffer {
    ...
    //Read raw YUV data
    video_frame->data[0] = CVPixelBufferGetBaseAddressOfPlane(pixelBuffer, 0);                                // Y
    video_frame->data[1] = CVPixelBufferGetBaseAddressOfPlane(pixelBuffer, 1);       // U
    video_frame->data[2] = CVPixelBufferGetBaseAddressOfPlane(pixelBuffer, 2);  // V
    // PTS
//    video_frame->pts = frameCount++;
    video_frame->pts = video_frame_count;
    // Encode
    int got_picture = 0;
    int ret = 0;
    ...
    ret = avcodec_send_frame(vc, video_frame);
    if (ret != 0){
